...
### 🎯 多维表格式设计 (v1.3.0)
- 📊 **一体化表格** - 所有筛选和操作集成在表格内，无需侧边栏
- 🔍 **表头筛选** - 搜索、状态、分类筛选全部在表头完成
- 📋 **全宽视图** - 100% 宽度利用，显示更多信息
- 🎯 **操作集中** - 所有按钮集中在表格上方，分组清晰
- ⚡ **即时反馈** - 筛选结果实时显示，一键清除所有筛选

### 核心功能
- 📤 **导入 OPML/JSON** - 支持拖拽上传或点击选择，支持 OPML 和 JSON 格式
- ✏️ **在线编辑** - 修改标题、分类、链接等，表格内直接编辑
- 🗂️ **分类管理** - 轻松移动订阅源到不同分类，支持手动添加新分类
- 📥 **导出功能** - 导出全部或仅导出选中的订阅源，支持 OPML 和 JSON 格式
- 👁️ **导出预览** - 导出前预览订阅源列表，选择导出格式
- 🧹 **去重功能** - 自动检测并删除重复的订阅源

### 高级功能
- ↩️ **撤销/重做** - 支持撤销/重做所有编辑操作（最多50步）
- ✅ **RSS 验证** - 真实网络验证 RSS 链接有效性，自动获取更新时间
- 📅 **更新时间** - 显示 RSS 源最后更新时间（智能相对时间格式）
- 🔄 **表头排序** - 点击表头按状态、分类、标题、链接、更新时间排序
- 📝 **批量编辑** - 批量修改多个订阅源的分类
- ⌨️ **快捷键** - 完整的键盘快捷键支持（Ctrl+Z/Y/S/A）

### UI/UX
- 🌓 **深色模式** - 完整的深色主题支持，自动跟随系统
- 🎨 **现代设计** - 渐变、玻璃态、阴影等现代视觉效果
- 🚀 **流畅动画** - 平滑过渡和微交互提升体验
